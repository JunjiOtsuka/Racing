...
{
	// [HideInInspector]
	public CarSO m_CarSO;
	// [HideInInspector]
	public CharacterSO m_CharSO;
	...
	public float driveForceBase;			
	public float driveForceTempBoost;			
	[HideInInspector]
	public float driveForceDefault;			
	...
	public float brakingVelFactorBase;
	public float brakingVelTempBoost;
	public float brakingVelFactorDefault;
	public float angleOfRoll = 30f;			//The angle that the ship "banks" into a turn
	public float angleOfRollBase;			
	public float angleOfRollTempBoost;			
	[HideInInspector]
	public float angleOfRollDefault;
	...
	public float terminalVelocityBase;
	public float terminalVelocityTempBoost;
	[HideInInspector]
	public float terminalVelocityDefault;   
	...
	public float driveForceMultiplier = 0f;
	public float MaxDriveForceMultiplier = 0.5f;
	public float angleOfRollMultiplier = 0f;
	public float MaxAngleOfRollMultiplier = 0.5f;
	public float terminalVelocityMultiplier = 0f;
	public float MaxTerminalVelocityMultiplier = 0.5f;
	public float brakeMultiplier = 0f;
	public float MaxBrakeMultiplier = 0.5f;

	public Rigidbody rigidBody;					//A reference to the ship's rigidbody
	public MyPlayerInput input;						//A reference to the player's input					
	public PlayerInput inputTest;						//A reference to the player's input					
    ...
    public static event Action UpgradeToExecute;

	[Header("Increment Upgrade Multiplier by %")]
	public float IncrementEngineUpgrade;
	public float IncrementAngleOfRollUpgrade;
	public float IncrementTerminalVelocityUpgrade;
	public float IncrementBrakeUpgrade;
	public float IncrementProjCDUpgrade;
	...
	void Start()
	{
		...
		driveForceDefault = driveForce;
		driveForceBase = driveForce;
		angleOfRollDefault = angleOfRoll;
		angleOfRollBase = angleOfRoll;
		terminalVelocityDefault = terminalVelocity;
		terminalVelocityBase = terminalVelocity;
		brakingVelFactorDefault = brakingVelFactor;
		brakingVelFactorBase = brakingVelFactor;
	}
	...
	void FixedUpdate()
	{
		...
		if (m_PV.IsMine) 
		{
			...
			m_PV.RPC("CalculateHover", RpcTarget.All);
			m_PV.RPC("CalculatePropulsion", RpcTarget.All);
			// CalculateHover();
			// CalculatePropulsion();
		}
	}
	...
	void UpdateCar()
	{
		if (m_CarSO == null) return;
		Debug.Log(m_CarSO.DriveForce);
	}

	void UpdateChar()
	{
		if (m_CharSO == null) return;
		Debug.Log(m_CharSO.CharacterPrefabName);
	}
	...
	void CalculateHover()
	{
		...
		float projectionAngle = Vector3.Angle(Vector3.up, groundNormal);
		if (projectionAngle > 45) {
			onSlope = true;
		} else {
			onSlope = false;
		}
	...
	void CalculatePropulsion()
	{
		...
		float rotationTorqueY = input.rudder - rigidBody.angularVelocity.y;
		//Apply the torque to the ship's Y axis
		rigidBody.AddRelativeTorque(0f, rotationTorqueY, 0f, ForceMode.VelocityChange);
		...
		if(inputTest.currentControlScheme == "KeyboardMouse") 
		{
			...
			if (MyPlayerInput.BrakeKey.IsPressed())
		...
		if(inputTest.currentControlScheme == "Gamepad") 
		{
			if (!MyPlayerInput.GasKey.IsPressed())
				rigidBody.velocity *= slowingVelFactor;
			...
			//Calculate and apply the amount of propulsion force by multiplying the drive force
			//by the amount of applied thruster and subtracting the drag amount
			float propulsion = driveForce * 1 - drag * Mathf.Clamp(speed, 0f, terminalVelocity);
			rigidBody.AddForce(transform.forward * propulsion, ForceMode.Acceleration);
		}
	}
    ...
    {
		if (this.tag == "Projectile" && collision.gameObject.tag == "Wall") {
    ...
    private void Bounce(Vector3 collisionNormal)
    {
        ...
        var direction = Vector3.Reflect(rigidBody.velocity.normalized, collisionNormal);

        Debug.Log("Out Direction: " + direction);
        rigidBody.velocity = direction * Mathf.Max(speed, 10f);
    }
	...
	public void UpdateCurrentStatusBoost()
    {
        //total = [permanent upgrade] + [buff + nerf]
        //ex) engine = (accumulated base engine * multiplier) + engine buff/nerf
		driveForce = driveForceBase + driveForceTempBoost;
		terminalVelocity = terminalVelocityBase + terminalVelocityTempBoost;
		angleOfRoll = angleOfRollBase + angleOfRollTempBoost;
		brakingVelFactor =  brakingVelFactorBase + brakingVelTempBoost;
    }
}